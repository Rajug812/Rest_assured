...
public class google_maps {
	...
	public void tc1_post() {
		...
		given().contentType(ContentType.JSON).accept(ContentType.JSON).body(js.toJSONString()).when().post("maps/api/place/add/json/").then().statusCode(200).log().all();
	...
	public void tc2_delete() {
		...
		RestAssured.baseURI=url;
	    js.put("place_id",str); 
	    given().queryParam("Key", "qaclick123").contentType(ContentType.JSON).accept(ContentType.JSON).body(js.toJSONString()).when().delete("maps/api/place/delete/json/").then().statusCode(200).log().all();
